...
export function generateMock(type:ToolKey,topic:string):ToolResult[]{
 ...
 return ["Instagram Reels","카드뉴스","Threads","뉴스레터","짧은 SNS"].map((badge,i)=>({title:`${t} · ${badge}`,badge,body:i===0?`첫 3초: “이거 직접 써보니 달랐어요.”\n화면: 사용 전후 비교\nCTA: 자세한 후기는 블로그에서 확인하세요.`:badge==="Threads"?`이거 나만 이제 알았나? 😉\n\n${t} 관련해서 직접 확인한 내용을 정리해봤어. 보기 전에는 다 비슷할 줄 알았는데, 실제로 살펴보니 체크할 부분이 있더라고. 원문에 있는 경험과 사실만 골라 짧게 나누니까 훨씬 이해하기 쉬웠어.\n\n스친들은 이런 아이템 고를 때 제일 먼저 뭘 확인해? ❓`:`${t}의 핵심 경험을 ${badge} 독자에게 맞게 짧고 자연스럽게 재구성한 초안입니다.`}));
}
